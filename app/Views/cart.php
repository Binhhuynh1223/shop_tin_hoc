...
<main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-8">
    ...
    <section>
        <div class="flex items-center justify-between mb-6">
            ...
            <a id="checkout-button" onclick="openModal()" class="inline-block px-5 py-3 rounded-md bg-indigo-600 text-white font-medium hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 <?= $cart && $cart->items->count() ? '' : 'hidden' ?>">Đặt hàng</a>
        </div>
    ...
    <div id="checkout-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            <div class="p-6 border-b flex justify-between items-center">
                <h2 class="text-xl font-bold">Thông tin đặt hàng</h2>
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    <h3 class="font-semibold mb-4">Sản phẩm:</h3>
                    <div class="space-y-4">
                        <?php foreach (($cart->items ?? []) as $item): ?>
                            <div class="flex justify-between items-center border-b pb-2">
                                <div class="flex items-center gap-4">
                                    <img src="<?= htmlspecialchars($item->product->image_url) ?>" alt="<?= htmlspecialchars($item->product->product_name) ?>" class="w-16 h-16 object-cover rounded-md" />
                                    <div>
                ...
                <div>
                    ...
                    <form id="checkout-form" class="space-y-4">
                        ...
                        <button type="submit" id="checkout-submit-button" class="w-full bg-indigo-600 text-white py-3 rounded-md font-medium hover:bg-indigo-700">Xác nhận đặt hàng</button>
                    </form>
                </div>
            </div>
        </div>
